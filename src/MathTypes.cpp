...
Mat4xn GetOrthoProjectionMat(float l, float r, float b, float t, float n, float f) {
    Mat4xn orthoMat(4);
    orthoMat.set(0, 0, 2 / (r - l));
    orthoMat.set(1, 1, 2 / (r - b));
    orthoMat.set(2, 2, - 2 / (f - n));
    orthoMat.set(0, 3, - (r + l) / (r - l));
    orthoMat.set(1, 3, - (t + b) / (t - b));
    orthoMat.set(2, 3, - (f + n) / (f - n));
    orthoMat.set(3, 3, 1);
    return orthoMat;
}
...
Mat4xn GetProjectionTransMat(float l, float r, float b, float t, float n, float f) {
    Mat4xn orthoMat = GetOrthoProjectionMat(l, r, b, t, n, f);
    Mat4xn persp2OrthoMat = GetPersp2OrthoMat(n, f);
...
Mat4xn MVPTrans(const Vec3& resize, const Vec3& rotate, const Vec3& move,
    ...
    Mat4xn projMat = GetProjectionTransMat(l, r, b, t, n, f);
